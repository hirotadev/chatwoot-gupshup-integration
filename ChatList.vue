...
// chatsOnView is o store the chats that are currently visible on the screen,
...
const getMyTeams = useMapGetter('teams/getMyTeams');
...
const conversationFilters = computed(() => {
  ...
  if (currentUser.value.role === 'agent' && getMyTeams.value && getMyTeams.value.length > 0) {
    return {
      ...baseFilters,
      filterPayload: [{
        attribute_key: "team_id",
        filter_operator: "equal_to",
        values: getMyTeams.value && getMyTeams.value.length > 0 ? getMyTeams.value.map(team => team.id) : [0],
...
const pageSize = ref(25); // Tamanho da página definido explicitamente
const totalItems = ref(0); // Total de itens disponíveis
const currentOffset = ref(0); // Offset atual para controle de paginação

const showEndOfListMessage = computed(() => {
  return (
    conversationList.value.length > 0 &&
    conversationList.value.length >= totalItems.value &&
...
// ---------------------- Methods -----------------------
async function getTeamsUser() {
  return await useMapGetter('teams/getMyTeams');
}
...
function updateTeamIdValues(payload) {
  let teamIdFound = false;

  const updatedPayload = payload.map(item => {
    if (item.attribute_key === 'team_id') {
      teamIdFound = true;
      // Modifique o índice `values` conforme necessário
      item.values = getMyTeams.value.map(team => team.id);
    }
    return item;
  });

  // Se não encontrou nenhum `team_id`, adiciona um novo objeto ao array
  if (!teamIdFound) {
    updatedPayload.push({
      attribute_key: "team_id",
      filter_operator: "equal_to",
      values: getMyTeams.value.map(team => team.id),
      custom_attribute_type: ""
    });
  }
...
async function fetchFilteredConversations(payload) {
  if (chatListLoading.value || showEndOfListMessage.value) {
    return;
  }

  let page = currentFiltersPage.value + 1;
  const updatedPayload = updateTeamIdValues(payload);

  try {
    const response = await store.dispatch('fetchFilteredConversations', {
      queryData: filterQueryGenerator(updatedPayload),
      page,
    });

console.log('response1', response);
console.log(conversationStats.value);

    // Atualiza o total de itens com base no meta.count da resposta
    totalItems.value = response.meta?.all_count || 0;
    currentOffset.value = (page - 1) * pageSize.value;
...
function fetchConversations() {
    ...
    const additionalFilter = conversationFilters.value?.filterPayload?.[0];
    if (additionalFilter) {
      baseFilter.query_operator = "and";
    }
  ...
  // Atualiza o total de itens com base no meta.count da resposta
  totalItems.value = conversationStats.value.allCount || 0;
  currentOffset.value = (page - 1) * pageSize.value;
...
function handleScroll() {
  ...
  console.log('scrollHeight - (scrollTop + clientHeight) < threshold');
  console.log(scrollHeight - (scrollTop + clientHeight) < threshold);
  console.log('!chatListLoading.value');
  console.log(!chatListLoading.value);
  console.log('!showEndOfListMessage.value');
  console.log(!showEndOfListMessage.value);
  console.log('conversationList.value.length < totalItems.value');
  console.log(conversationList.value.length < totalItems.value);
...
const isDataReady = computed(() => {
  return currentUser.value && getMyTeams.value;
});
...
watch(chatLists, (newValue) => {
  if (newValue && Array.isArray(newValue)) {
    chatsOnView.value = [...newValue];
  }
});
